...
class ResizeAndScale(object):
    ...
    def __call__(self, sample):
        ...
        for msk_key in msk_keys:
            scale_mult = (1. / scale) if 'depth' in msk_key else 1
            sample[msk_key] = (scale_mult *
                               cv2.resize(
                                   sample[msk_key],
                                   None,
                                   fx=scale,
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        for msk_key in msk_keys:
            sample[msk_key] = (torch.from_numpy(sample[msk_key])
                               .to(KEYS_TO_DTYPES[msk_key]))
        return sample
