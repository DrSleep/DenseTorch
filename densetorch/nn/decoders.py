...
class MTDLv3plus(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(MTDLv3plus, self).__init__()
        ...
        self.clfs = nn.ModuleList(
            [
                conv1x1(agg_size, output_classes, bias=True)
                for output_classes in num_classes
            ]
        )
    ...
    def forward(self, xs):
        ...
        dec = torch.cat(dec, dim=1)
        dec = self.dec(dec)
        return [clf(dec) for clf in self.clfs]
